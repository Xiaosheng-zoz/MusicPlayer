...
`dist/1.1.0/MusicPlayer.exe` —— 双击即可，不需要预先安装 .NET。
首次运行会把运行库解压到临时目录（约 3 秒），之后启动约 0.7 秒。

打开后点左侧「文件夹」，把音乐文件夹的路径粘进去（例如 `E:\LocalMusic`），回车或点「打开」。
**下次启动会自动打开这个文件夹**，不用再粘一次。不想要这个行为就到左侧「设置」里关掉。
...
dotnet publish src/MusicPlayer.App -c Release -f net10.0-windows10.0.19041.0 -o dist/1.1.0
```
...
| 拖动进度 / 调音量 | 播放条上的滑块 |
| 改设置 | 点左侧「设置」；目前可开关「记住上次打开的文件夹」 |

左侧分区：**音乐**（搜索 + 歌曲列表）、**文件夹**（选文件夹并扫描，只报数量）、**设置**。**歌单**是占位，后续再做。
