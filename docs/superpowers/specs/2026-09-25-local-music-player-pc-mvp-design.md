...
- **播放方式**：顺序播放 / 随机播放 / 单曲循环，点按钮循环切换。按钮用 emoji 表示当前模式：`➡️` 顺序、`🔀` 随机、`🔂` 单曲循环（这三个字符走系统 emoji 字体，带彩色底板，这是接受的）。鼠标悬停显示文字说明，写明当前模式以及点一下会切到哪种。随机播放每次换歌都从"除当前这首之外"的曲目里挑，不会连着播同一首，也不会停；单曲循环只影响"播完自动前进"，显式按上一首/下一首仍然换歌
