...
- **侧边栏**：音乐 / 文件夹 / 歌单 / 设置（"正在播放"已去掉）
- **音乐**：顶部是搜索栏（按歌名和歌手做不区分大小写的包含匹配），下面才是歌曲列表。搜索只改变显示哪些行，**不改动库本身**——序号和播放队列都仍以整张库为准（否则搜到第 3 条、点它，播的会是队列里的第 3 首而不是它）
- **文件夹**：只管扫描。路径输入框 + 打开 + 浏览，扫描成功后**只报歌曲数量**（例如「228 首」），歌曲列表不在这里显示
- **序号不随搜索变化**：歌曲行最左边是在**整张库**里的序号，搜索过滤后不会重新编号成 1..N
...
- **标题栏**：左上角显示 **应用图标 + 软件名 + 版本号**（如 `MusicPlayer 1.1.0`），右上角是最小化 / 最大化 / 关闭。用 MAUI 自带的 `TitleBar` 控件实现——它会自己处理"内容延伸到标题栏"和拖拽区域，不用手写 WinRT 平台代码。版本号的唯一来源是 csproj 里的 `<Version>`
- **顶部路径输入**：工具栏第二行是「路径输入框 + 打开 + 浏览…」。之所以不只依赖系统文件夹对话框：那个对话框**只列文件夹、不显示文件**，用户没法在里面确认某个文件夹到底有没有音乐。可以直接把路径粘进输入框，回车即扫描

### 5.1 MVP 阶段侧边栏的处理

侧边栏按最终形态摆放。**"音乐""文件夹""设置"是活的**，点哪个哪个高亮、右区切到对应内容，默认进"音乐"。"歌单"仍显示为禁用态（灰色、不可点），等后续做歌单功能时再接上。
